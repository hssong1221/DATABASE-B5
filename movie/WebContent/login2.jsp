...
<%!
	...
	public boolean isduplicatenum(String num1 ,String num2) throws Exception{
		Connection conn = null;
		Context init = new InitialContext();
		DataSource ds = (DataSource) init.lookup("java:comp/env/jdbc/OracleDB");
		conn = ds.getConnection();
		try{
			String sql2 = "select num1,num2 from client";
			PreparedStatement stmt2 = conn.prepareStatement(sql2);
			ResultSet rs2 = stmt2.executeQuery();

		    
			while(rs2.next()) {
				if(rs2.getString("num1").equals(num1)&&rs2.getString("num2").equals(num2)){
					return true;
				}
			}
		}catch(Exception e){
...
%>
	...
	<%}
	else if(isduplicatenum(num1,num2)){%>
		<script>
			alert("이미 존재하는 회원입니다.");
			history.back(-1);
		</script>
